...
static void bench_tweak_setup(void* arg) {
    ...
    CHECK(secp256k1_ec_pubkey_create(data->ctx, &data->pubkey_obj, seckey));
}

static void bench_pk_tweak_mul_run(void *arg, int iters) {
    int i;
    unsigned char tweak[32] = {2};
    bench_data *data = (bench_data*)arg;

    for (i = 0; i < iters; i++) {
        unsigned char pub33[33];
        size_t len = 33;
        CHECK(secp256k1_ec_pubkey_tweak_mul(data->ctx, &data->pubkey_obj, tweak));
        CHECK(secp256k1_ec_pubkey_serialize(data->ctx, pub33, &len, &data->pubkey_obj, SECP256K1_EC_COMPRESSED));
        memcpy(tweak, pub33 + 1, 32);
    }
}
...
int main(int argc, char** argv) {
    ...
    char* valid_args[] = {"ecdsa", "verify", "ecdsa_verify", "sign", "ecdsa_sign", "ecdh", "recover",
                         ...
                         "keygen", "ec_keygen", "tweak", "ec_pk_tweak_mul", "ellswift", "encode",
...
/* Check if the user tries to benchmark optional module without building it */
    ...
    if (d || have_flag(argc, argv, "ec") || have_flag(argc, argv, "tweak") || have_flag(argc, argv, "ec_pk_tweak_mul")) run_benchmark("ec_pk_tweak_mul", bench_pk_tweak_mul_run, bench_tweak_setup, NULL, &data, 10, iters);
