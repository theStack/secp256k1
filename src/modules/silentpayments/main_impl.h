...
static void secp256k1_silentpayments_create_shared_secret(const secp256k1_context *ctx, unsigned char *shared_secret33, const secp256k1_ge *public_component, const secp256k1_scalar *secret_component) {
     ...
     * impossible at this point considering we have already validated the public component
     * (scan public key for sender, public key sum for the receiver) and secret component
     * (input hash * secret key sum for the sender, input hash * scan secret key for receiver)
     * each at the respective call-sites.
     */
...
int secp256k1_silentpayments_label_parse(const secp256k1_context* ctx, secp256k1_silentpayments_label* label, const unsigned char *in33) {
    secp256k1_ge ge;
...
int secp256k1_silentpayments_label_serialize(const secp256k1_context* ctx, unsigned char *out33, const secp256k1_silentpayments_label* label) {
    secp256k1_ge ge;
...
int secp256k1_silentpayments_recipient_create_label(const secp256k1_context *ctx, secp256k1_silentpayments_label *label, unsigned char *label_tweak32, const unsigned char *scan_key32, uint32_t m) {
    secp256k1_sha256 hash;
