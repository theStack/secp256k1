...
static void test_label_api(void) {
     ...
     */
    {
        ...
        CHECK(secp256k1_silentpayments_label_parse(CTX, &neg_spend_label, neg_spend_label_ser));
...
void run_silentpayments_test_vector_receive(const struct bip352_test_vector *test) {
    ...
    for (i = 0; i < test->num_labels; i++) {
        ...
        CHECK(secp256k1_silentpayments_label_serialize(CTX, cache_entry->label, &label));
