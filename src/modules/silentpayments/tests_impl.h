...
void run_silentpayments_test_vector_receive(const struct bip352_test_vector *test, const struct bip352_receiving_subtest *subtest) {
...
void run_silentpayments_test_vectors(void) {
    ...
    for (i = 0; i < ARRAY_SIZE(bip352_test_vectors); i++) {
        ...
        for (j = 0; j < test->num_receiving_subtests; j++) {
            run_silentpayments_test_vector_receive(test, &test->receiving_subtests[j]);
        }
    }
}
