...
for test_nr, test_vector in enumerate(test_vectors):
    ...
    for recipient_B_scan, recipient_spend_data in groups.items():
        ...
        for recipient_B_spend in recipient_spend_data:
            output_xonly, _, _ = secp256k1_glue.silentpayments_create_output_pubkey(shared_secret, recipient_B_spend[0], k)
    ...
    for subtest_nr, receive_data in enumerate(test_vector['receiving']):
        ...
        while True:
            ...
            for output_to_check in outputs_to_check:
                found_sth = False
                output_xonly_pubkey, output_pubkey, t_k = secp256k1_glue.silentpayments_create_output_pubkey(shared_secret, spend_pubkey, k)
                if output_xonly_pubkey.hex() == output_to_check:
                    ...
                    outputs_privkeys.append(t_k.hex())
                    k += 1
                    found_sth = True
                    break
                # now, scan for labels
                label1, label2 = secp256k1_glue.silentpayments_create_scan_labels(output_pubkey, bytes.fromhex(output_to_check))
                label_found = None
