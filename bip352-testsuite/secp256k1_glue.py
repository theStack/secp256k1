...
"""
secplib.secp256k1_xonly_pubkey_from_pubkey.argtypes = [c_voidp, c_voidp, c_voidp, c_voidp]
secplib.secp256k1_xonly_pubkey_from_pubkey.restype = c_int
def xonly_pubkey_from_pubkey(pubkey_ser):
    result_xpubkey = create_string_buffer(SECP256K1_XONLY_PUBKEY_STRUCT_SIZE)
    retval = secplib.secp256k1_xonly_pubkey_from_pubkey(
        secp_context, result_xpubkey, None, pubkey_parse(pubkey_ser))
    assert retval == 1
    return xonly_pubkey_serialize(result_xpubkey)
...
secplib.secp256k1_silentpayments_create_output_pubkey.argtypes = [
    c_voidp, c_voidp, c_voidp, c_voidp, c_voidp, c_voidp, c_uint
]
secplib.secp256k1_silentpayments_create_output_pubkey.restype = c_int
def silentpayments_create_output_pubkey(shared_secret, receiver_spend_pubkey, k):
    result_xonly_pubkey = create_string_buffer(SECP256K1_XONLY_PUBKEY_STRUCT_SIZE)
    result_pubkey = create_string_buffer(SECP256K1_PUBKEY_STRUCT_SIZE)
    result_t_k = create_string_buffer(32)
    # convert receiver pubkey to secp256k1 object
    receiver_spend_pubkey_obj = pubkey_parse(receiver_spend_pubkey)

    retval = secplib.secp256k1_silentpayments_create_output_pubkey(
        secp_context, result_xonly_pubkey, result_pubkey, result_t_k, shared_secret, receiver_spend_pubkey_obj, k)
    assert retval == 1

    return xonly_pubkey_serialize(result_xonly_pubkey), pubkey_serialize(result_pubkey), bytes(result_t_k)
...
"""
secplib.secp256k1_silentpayments_create_scan_labels.argtypes = [
    c_voidp, c_voidp, c_voidp, c_voidp, c_voidp
]
secplib.secp256k1_silentpayments_create_scan_labels.restype = c_int
def silentpayments_create_scan_labels(P_output, tx_output):
    result1_pubkey = create_string_buffer(SECP256K1_PUBKEY_STRUCT_SIZE)
    result2_pubkey = create_string_buffer(SECP256K1_PUBKEY_STRUCT_SIZE)
    P_output_obj = pubkey_parse(P_output)
    tx_output_obj = xonly_pubkey_parse(tx_output)
    retval = secplib.secp256k1_silentpayments_create_scan_labels(
        secp_context, result1_pubkey, result2_pubkey, P_output_obj, tx_output_obj)
    assert retval == 1

    return pubkey_serialize(result1_pubkey), pubkey_serialize(result2_pubkey)
...
def silentpayments_create_output_seckey(shared_secret, receiver_spend_seckey, t_k, label_tweak=None):
