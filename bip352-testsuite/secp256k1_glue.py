...
def silentpayments_create_private_tweak_data(plain_seckeys, xonly_seckeys, outpoints_hash):
    ...
    retval = secplib.secp256k1_silentpayments_create_private_tweak_data(
        ...
        ser_xonly_seckeys, len(xonly_seckeys), outpoints_hash)
    assert retval == 1
...
def silentpayments_create_tweak_data(plain_pubkeys, xonly_pubkeys, outpoints_hash):
    ...
    retval = secplib.secp256k1_silentpayments_create_public_tweak_data(
        ...
        ser_xonly_pubkeys, len(xonly_pubkeys), outpoints_hash)
    assert retval == 1
...
def silentpayments_create_output_pubkey(shared_secret, receiver_spend_pubkey, k):
    ...
    retval = secplib.secp256k1_silentpayments_create_output_pubkey(
        secp_context, result_xpubkey, shared_secret, receiver_spend_pubkey_obj, k, None)
    assert retval == 1
...
def silentpayments_create_output_seckey(shared_secret, receiver_spend_seckey, k):
    result_seckey = create_string_buffer(32)
    retval = secplib.secp256k1_silentpayments_create_output_seckey(
        secp_context, result_seckey, shared_secret, receiver_spend_seckey, k, None)
    assert retval == 1
